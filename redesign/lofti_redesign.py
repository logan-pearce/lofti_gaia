...
class Fitter(object):
    ...
    def __init__(self, sourceid1, sourceid2, mass1, mass2, Norbits = 100000, results_filename = None):
        ...
        if not results_filename:
            self.results_filename = 'FitResults.'+time.strftime("%Y.%m.%d.%H.%M.%S")+'.pkl'
...
class FitOrbit(object):
    ...
    def fitorbit(self, save_results_every_X_loops = 100):
        ...
        if self.rv[0] != 0:
            self.results = Results(orbits = parameters[accepted], limit_lan = False, limit_aop = False)
        else:
            self.results = Results(orbits = parameters[accepted], limit_lan = True, limit_aop = False)
        ...
        while number_orbits_accepted < self.Norbits:
            ...
            if loop_count % save_results_every_X_loops == 0 and self.write_results:
                # every Xth loop, export results to pickle file:
                self.results.SaveResults(self.results_filename)
